...
msg.innerHTML = ` <div style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:8px;padding:12px;margin-top:8px;"> <div style="font-weight:600;color:#15803d;margin-bottom:6px;">✓ Preceptor agregado</div> <div style="font-size:13px;color:#374151;"> <b>Email:</b> ${email}<br> <b>Contraseña temporal:</b> ${pass}<br> <span style="color:#6b7280;">Compartí estos datos con el preceptor para que pueda ingresar.</span> </div> </div>`;
...
document.getElementById(“app”).innerHTML = `<div class="panel-wrap"> <header class="panel-header"> <div> <h1 class="panel-title">Asistencia QR</h1> <p class="panel-sub">${IFD} · ${p.nombre}</p> </div> <div class="row-gap" style="margin:0;gap:6px;"> ${cursos.length > 1 ?`
...
</select>`:`<span style="font-weight:500;font-size:14px;">${cursoActivo}</span>`} ${fromAdmin ? `<button class="btn-outline sm" onclick="renderAdminPanel()">← Admin</button>`:`<button class="btn-outline sm" onclick="logout()">Cerrar sesión</button>`}
</div>
</header>
...
app.innerHTML = ` <div class="phone-wrap"> <div class="phone-header"> <div class="logo-pill">${IFD}</div> <h1 class="phone-title">Fuera de horario</h1> </div> <div class="phone-body"> <div class="alert-error" style="text-align:center;padding:1.5rem;"> <div style="font-size:32px;margin-bottom:12px;">🕐</div> <div style="font-weight:600;">El registro está disponible de 13:30 a 18:00 hs</div> <div style="margin-top:10px;font-size:13px;">Hora actual: ${horaStr} hs</div> </div> </div> </div>`;
return;
}
...
app.innerHTML = ` <div class="phone-wrap"> <div class="phone-header"> <div class="logo-pill">${IFD}</div> <h1 class="phone-title">Marcar presencia</h1> <p class="phone-sub">${formatearFecha(fechaId)}</p> </div> <div class="phone-body"> <div class="alert-success" style="text-align:center;"> <div style="font-size:32px;margin-bottom:8px;">✓</div> <div style="font-size:16px;font-weight:600;">¡Ya registraste tu presencia hoy!</div> <div style="margin-top:8px;font-size:14px;opacity:0.8;">${nombre}</div> </div> <p style="text-align:center;font-size:13px;color:#9ca3af;margin-top:1rem;">Solo una vez por día por dispositivo.</p> </div> </div>`;
return;
}

app.innerHTML = ` <div class="phone-wrap"> <div class="phone-header"> <div class="logo-pill">${IFD}</div> <h1 class="phone-title">Marcar presencia</h1> <p class="phone-sub" id="scan-sub">Cargando...</p> </div> <div id="scan-body" class="phone-body"> <p style="text-align:center;color:#6b7280;font-size:14px;">Cargando lista...</p> </div> </div>`;
...
document.getElementById(“scan-body”).innerHTML = ` <div class="form-group"> <label class="form-label">Seleccioná tu nombre</label> <select id="alumno-sel" class="form-select"> <option value="">— Elegí tu nombre —</option> ${opciones} </select> </div> <button class="btn-big" id="btn-marcar" onclick="marcarPresente('${precId}','${cursoId}','${fechaId}')" disabled> Marcar presente </button> <div id="scan-msg"></div> <div class="presentes-count">${presentes.length} de ${alumnos.length} registrados</div>`;
...
document.getElementById(“scan-body”).innerHTML = ` <div class="alert-success" style="text-align:center;"> <div style="font-size:40px;margin-bottom:8px;">✓</div> <div style="font-size:17px;font-weight:600;">¡Presencia registrada!</div> <div style="margin-top:10px;font-size:15px;">${nombre}</div> <div style="margin-top:4px;font-size:13px;opacity:0.8;">${formatearFecha(fechaId)} · ${hora}</div> </div> <p style="text-align:center;font-size:13px;color:#9ca3af;margin-top:1rem;">Ya podés cerrar esta página.</p>`;
});
}
...
if (window.*driveCallback) window.*driveCallback();
}
} catch(e) {}
...
const cursoFile = cursoActivo.replace(/[°\s]/g,”*”);
const nombre    = “Asistencia_IFD12*” + cursoFile + “_” + YEAR + “.xlsx”;
