...
return /FBAN|FBAV|Instagram|Twitter|LinkedIn|WebView|wv|FB_IAB/.test(ua) ||
...
// En WebView mostrar email por defecto, Google no funciona
const defaultTab = webView ? “email” : “google”;
...
```
    ${webView ? `
    <div style="background:#fef3c7;border:1px solid #fcd34d;border-radius:8px;padding:10px 12px;margin-bottom:1rem;font-size:13px;color:#92400e;">
      Para usar Google, abrí esta página en <strong>Safari</strong> o <strong>Chrome</strong>.<br>
      O ingresá con email y contraseña.
    </div>` : ''}
    ...
    <div id="login-tabs" style="display:flex;gap:0;margin-bottom:1.25rem;border:1px solid #e5e7eb;border-radius:8px;overflow:hidden;">
      <button id="tab-google-btn" onclick="switchLoginTab('google')"
        style="flex:1;padding:9px;border:none;background:${defaultTab==='google'?'#1A3A5C':'#f3f4f6'};color:${defaultTab==='google'?'#fff':'#374151'};font-size:13px;font-weight:500;cursor:pointer;">
        Google
      </button>
      <button id="tab-email-btn" onclick="switchLoginTab('email')"
        style="flex:1;padding:9px;border:none;background:${defaultTab==='email'?'#1A3A5C':'#f3f4f6'};color:${defaultTab==='email'?'#fff':'#374151'};font-size:13px;font-weight:500;cursor:pointer;">
        Email y contraseña
      </button>
    </div>

    <!-- Google -->
    <div id="login-google" style="display:${defaultTab==='google'?'block':'none'};">
    ...
    <!-- Email -->
    <div id="login-email" style="display:${defaultTab==='email'?'block':'none'};">
