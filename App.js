...
async function obtenerEstadoAsistencia(cursoId, alumno) {
  const [fechasSnap, presentesSnap] = await Promise.all([
    db.ref(dbPath(currentData.id, "cursos", cursoId, "fechas")).once("value"),
    db.ref(dbPath(currentData.id, "cursos", cursoId, "presentes")).once("value")
  ]);
  const fechas   = fechasSnap.val()    || {};
  const presentes = presentesSnap.val() || {};
  ...
  for (const fid in fechas) {
    const tipo = fechas[fid]?.tipo;
    if (tipo && tipo !== "normal") continue; // ignorar feriados/jornadas
    totalDias++;
    const pd = presentes[fid] ? Object.values(presentes[fid]) : [];
...
async function obtenerCarpetaCertificados() {
  if (certificadosFolderId) return certificadosFolderId;

  const q = encodeURIComponent("name='Certificados' and mimeType='application/vnd.google-apps.folder' and trashed=false");
  const r = await fetch(
  ...
  if (data.files?.length) {
    certificadosFolderId = data.files[0].id;
  ...
  const c = await fetch("https://www.googleapis.com/drive/v3/files", {
    ...
    body: JSON.stringify({ name: "Certificados", mimeType: "application/vnd.google-apps.folder" })
  });
  const folder = await c.json();
  if (!folder.id) throw new Error("No se pudo crear la carpeta Certificados en Drive");
  certificadosFolderId = folder.id;
  return folder.id;
}

// ── SUBIR ARCHIVO A GOOGLE DRIVE ───────────────────────────────────
async function subirArchivoADrive(archivo, nombreArchivo, carpetaPadreId) {
  const meta = JSON.stringify({ name: nombreArchivo, parents: [carpetaPadreId] });
  const body = new FormData();
  body.append("metadata", new Blob([meta], { type: "application/json" }));
...
async function obtenerURLDescarga(archivoId) {
  const r = await fetch(
    `https://www.googleapis.com/drive/v3/files/${archivoId}?fields=webContentLink,webViewLink`,
    { headers: { Authorization: "Bearer " + gdriveToken } }
  );
  const data = await r.json();
...
async function guardarJustificacionFalta(cursoId, alumno) {
  ...
  if (archivo) {
    try {
      ...
      if (!gdriveToken) {
        try {
          await autenticarDrive("certificados");
        } catch (authErr) {
...
function toggleMotivo(element) {
  const motivo = element.nextElementSibling;
  const flecha = element.querySelector('span[style*="transition"]');
